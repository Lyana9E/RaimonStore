  ...
  <div class="container mx-auto p-4 sm:p-8">
    ...
    <div class="mb-6 flex flex-col sm:flex-row items-center gap-4">
      <aInputSearch
          ...
          @search="onSearch"
      />
      <div class="flex-col items-center gap-2 w-full sm:w-auto">
        <span class="text-primary whitespace-nowrap">مرتب‌سازی بر اساس قیمت:</span>
        <a-radio-group v-model:value="productStore.sortOrder" button-style="solid" class="flex items-center ">
          <a-radio-button value="asc" >کم به زیاد</a-radio-button>
          <a-radio-button value="desc">زیاد به کم</a-radio-button>
        </a-radio-group>

        <a-button v-if="productStore.sortOrder" @click="clearSortOrder" type="text" danger shape="circle" size="large">
          <template #icon>
    ...
    <aSpin :spinning="productStore.isLoading" size="large" tip="در حال بارگذاری محصولات...">
      ...
      <div v-else-if="productStore.filteredAndSortedProducts.length === 0" class="mt-8 text-center text-gray-500">
        <aEmpty description="محصولی یافت نشد." />
      </div>
